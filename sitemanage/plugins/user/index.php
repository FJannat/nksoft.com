...
if(isset($_GET['delete'])) {
	$delete = mysql_query("delete from user where id=".$_GET['delete']) or die(mysql_error());
}
...
<br />
<div style="font-size: 18px; font-weight: bold; color: #cccccc">&nbsp;&nbsp;User</div>
...
////		while($attorney = mysql_fetch_object($sqlAttorney)) {
		while($attorney = mysql_fetch_object($db->sql_result)) {
			...
			echo '<td style="text-align: left; background-color: #f1f1f1; border-bottom: 1px dashed #999999;">'.stripslashes($attorney->user_id).'</td>';
			echo '<td style="text-align: right; background-color: #f1f1f1; border-bottom: 1px dashed #999999;">'.stripslashes($attorney->user_level).'</td>';
